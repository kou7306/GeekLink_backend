...
import { qiitaOauthService, qiitaCallBackService } from "../services/qiitaService";

export const qiitaOauth = async (req: Request, res: Response) => {
  try {
    const client_id = process.env.QIITA_CLIENT_ID;
    if(client_id == undefined) return
    await qiitaOauthService(client_id);
